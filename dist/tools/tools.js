...
const mongo_1 = require("../utils/storage/mongo");
const hybridSearch_1 = require("../utils/search/hybridSearch");
...
async function getFigmaFile(fileKey, fileVersion, figmaToken) {
    const fileDoc = await (0, mongo_1.readFigmaFileCache)(fileKey, fileVersion);
    if (fileDoc) {
        return fileDoc;
    }
    const response = await axios_1.default.get(`https://api.figma.com/v1/files/${fileKey}`, {
    ...
    if (resJson) {
        await (0, mongo_1.writeFigmaFileCache)(resJson, fileKey, fileVersion);
    }
    return resJson;
}
async function queryFigmaFileNode(fileKey, query, topK, figmaToken, embeddings) {
